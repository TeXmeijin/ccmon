...
ccmon is an ambient TUI monitor for Codex sessions. It uses Codex's command hooks to capture session events into SQLite, then renders a real-time card grid via Bubble Tea. Written in Go with CGO (for go-sqlite3).
...
# Install hooks into Codex settings.json
./ccmon install

# Process a hook event from stdin (called by Codex, not manually)
./ccmon hook
...
```
Codex hooks → ccmon hook (stdin JSON) → SQLite → ccmon tui (300ms poll)
```
...
- `internal/hook/hook.go` — Parses Codex hook JSON payloads, extracts preview/headline/status, upserts session and inserts event into DB.
...
- **Source namespace**: Sessions are scoped by namespace (derived from `--source` flag or config dir basename), allowing multiple ccmon instances for different Codex configs (personal/work).
